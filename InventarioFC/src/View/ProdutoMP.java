 ...
 *
 */
public class ProdutoMP extends JFrame {
	 ...
	 */
	public ProdutoMP() {
		setTitle("Produto - Matéria Prima");
		...
		JButton btnTranforma = new JButton("Transformar em P.A.");
		btnTranforma.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				try {
					int resultado = JOptionPane.showConfirmDialog(null, "Transformar "+txtQuantidade.getText()+
					" peças em Produto Acabado de "+comboBox.getSelectedItem().toString()+" ?");
					if(resultado == JOptionPane.YES_OPTION){
						DAOProduto dpt = new DAOProduto();
						DAOProdutoAcabado dptpa = new DAOProdutoAcabado();
						int temp = dpt.busca_quantidade(comboBox.getSelectedItem().toString());
						int temppa = dptpa.busca_quantidade(comboBox.getSelectedItem().toString());
						int att = temp - Integer.parseInt(txtQuantidade.getText());
						int attpa = temppa + Integer.parseInt(txtQuantidade.getText());
						dpt.att_quantidade(comboBox.getSelectedItem().toString(), att);
						dptpa.att_quantidade(comboBox.getSelectedItem().toString(), attpa);
						carregarTabela();
					}
		...
		btnTranforma.setBounds(367, 189, 162, 23);
		contentPane.add(btnTranforma);
		...
		btnAdiciona.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				try {
					...
					" peças no estoque de "+comboBox.getSelectedItem().toString()+" ?");
					if(resultado == JOptionPane.YES_OPTION){
						DAOProduto dpt = new DAOProduto();
						int temp = dpt.busca_quantidade(comboBox.getSelectedItem().toString());
		...
		mntmInserir.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				InserirProduto ins = new InserirProduto();
				ins.setVisible(true);
		...
		mntmAtualizar.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				AtualizarMP att = new AtualizarMP();
				att.setVisible(true);
	...
	{			
	    ...
	    JFrame frame = new JFrame("Deleção de produto");
	    String delItem = (String) JOptionPane.showInputDialog(frame, 
	        ...
	        resultados[0]);
	    	if(delItem != null) {
	    		int resp = JOptionPane.showConfirmDialog(null, "O Produto será deletado de MP e PA!\nConfirma a deleção do item selecionado?");	
